...
def main(
    ...
    intermediate_output: Annotated[
        bool,
        typer.Option(
            "--intermediate-output",
            help="Save intermediate products (vision instructions and generated script)"
        ),
    ] = False,
    ...
    if output_folder:
        ...
        # Update all output paths to use the output folder
        output_file = str(output_dir / Path(output_file).name)
        if markdown_file:
            markdown_file = str(output_dir / Path(markdown_file).name)
        toc_output = str(output_dir / Path(toc_output).name)
        if toc_markdown:
            toc_markdown = str(output_dir / Path(toc_markdown).name)
        fixed_toc_output = str(output_dir / Path(fixed_toc_output).name)
        ...
        if refine_toc and extract_toc and toc_entries:
            ...
            try:
                ...
                if intermediate_output:
                    if output_folder:
                        instructions_file = str(Path(output_folder) / "toc-refinement-instructions.txt")
                    else:
                        instructions_file = "toc-refinement-instructions.txt"
                    print(f"Saving vision model instructions to {instructions_file}...")
                ...
                # Save generated script if intermediate output is requested
                if intermediate_output:
                    if output_folder:
                        script_file = str(Path(output_folder) / "toc-fix-script.py")
                    else:
                        script_file = "toc-fix-script.py"
                    print(f"Saving generated script to {script_file}...")
